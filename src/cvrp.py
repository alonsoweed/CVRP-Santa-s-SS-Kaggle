...
def random_sampling(solution, savings, current_best):
...
def remote_job(chunk_size, solution, deque_savings, best):
    ...
    probe = random_sampling(local_solution, deque_savings, local_best_cost)
...
def montecarlo_simulation(problem, ncpus,nprobes,servers):
    ...
    for _ in xrange(len(savings) - 10):
        ...
        if sol.feasibleSaving(saving):  
            ...
            unprocessed = sol
            no = 0 
            jobs = [job_server.submit(remote_job, (chunk_size, unprocessed, deq_savings, best_cost), (random_sampling,), ("random", "copy", "itertools")) for _ in xrange(ncpus)]
            ...
            yes = 0       
            jobs = [job_server.submit(remote_job, (chunk_size, processed, deq_savings, best_cost), (random_sampling,), ("random", "copy", "itertools")) for _ in xrange(ncpus)]  
            results = [job() for job in jobs]
            for (lbest_cost, lbest_path, lsum) in results:
                if lbest_cost < best_cost:
                    best_cost = lbest_cost
