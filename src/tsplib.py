from cvrp import problem_instance
import sys

def read_problem(filename):
    f = open(filename)
    cvrp = problem_instance()
    #lee el archivo con el formato .vrp. Coge el segmento de datos (nombre, tipo de problema, dimension, capacidad del vehiculo, metrica a usar, etc)
    while True:
        line = f.readline()
...
def write_solution(result,filename = None):
        (cost,paths) = result
        if filename == None:
            f = sys.stdout
        else:
